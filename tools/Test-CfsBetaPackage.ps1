...
if ([string]::IsNullOrWhiteSpace($Version)) { $Version = [string]$props.Project.PropertyGroup.CfsVersion }
if ([string]::IsNullOrWhiteSpace($Label)) { $Label = [string]$props.Project.PropertyGroup.CfsReleaseLabel }
if ([string]::IsNullOrWhiteSpace($Version) -or $Version -notmatch '^\d+\.\d+\.\d+$' -or [string]::IsNullOrWhiteSpace($Label)) {
    throw 'Pass a semantic -Version and non-empty -Label, or define them in Directory.Build.props.'
}
$identity = "CFS $Version $Label"
$packageName = "CFS-$Version-$Label-win-x64"
$releaseNotes = "RELEASE-NOTES-$Version-$($Label.ToUpperInvariant()).md"
$performanceReport = "CFS-$Version-$Label-performance.json"
$legacyPackage = $Version -eq '0.1.0'
...
$allowedTemplate = [IO.Path]::GetFullPath((Join-Path $folder 'ShellNew\CFS-Empty.cfs'))
$forbidden = @($files | Where-Object {
    ($_.Extension -in @('.pdb','.cs','.obj','.cfs','.log','.zip') -and $_.FullName -ne $allowedTemplate) -or
    $_.Name -match 'pasted-text|goal-objective|benchmark-workspace|\.codex'
})
...
Check "packaged executable identity is $identity" ($exeVersion -eq "$Version $Label")
foreach ($relative in @('README.md','BETA-NOTICE.txt','docs/BETA-QUICK-START.md',"docs/$releaseNotes",'docs/PERFORMANCE-BASELINE.md')) {
    Check "document identity: $relative" ((Get-Content -Raw -LiteralPath (Join-Path $folder $relative)).IndexOf($identity,[StringComparison]::Ordinal) -ge 0)
}
Check 'performance report identity' ((Get-Content -Raw -LiteralPath (Join-Path $folder "docs\$performanceReport")).IndexOf(('"Product": "' + $identity + '"'),[StringComparison]::Ordinal) -ge 0)
...
try {
    ...
    $unregisterIsScoped = if ($legacyPackage) {
        $unregisterDryRun.Contains('HKCU\Software\Classes\.cfs') -and $unregisterDryRun.Contains('HKCU\Software\Classes\CFS.Archive')
    }
    else {
...
try {
    foreach ($entry in $archive.Entries | Where-Object { -not $_.FullName.EndsWith('/') }) {
        ...
        $stream = $entry.Open(); try { $hash = [Security.Cryptography.SHA256]::Create(); try { $zipHashes[$name.Substring($prefix.Length)] = ([BitConverter]::ToString($hash.ComputeHash($stream))).Replace('-', '') } finally { $hash.Dispose() } } finally { $stream.Dispose() }
    }
}
