...
if ($version -ne '0.1.0' -or $label -ne 'Beta') { throw 'This pipeline is locked to CFS 0.1.0 Beta.' }
...
# Build the canonical portable release first. This also ensures the installer and
# portable archive contain the exact same application payload.
& (Join-Path $repoRoot 'tools\Publish-CfsPrototype.ps1')
...
if (-not (Test-Path -LiteralPath $publishedFolder) -or -not (Test-Path -LiteralPath $publishedZip)) {
    throw 'The portable publishing script did not create its expected outputs.'
}
