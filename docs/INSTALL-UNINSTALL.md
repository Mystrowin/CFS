# CFS 0.1.0 Beta installation and uninstall

## Recommended machine-wide setup

Run `CFS-0.1.0-Beta-Setup.exe`, approve the Windows administrator prompt, and follow the setup wizard. Setup installs the self-contained x64 application under `Program Files\CFS`, registers `.cfs` files for the machine, adds Start-menu and uninstall entries, and checks the `Client-ProjFS` Windows feature. A restart may be required after enabling ProjFS.
...
Extract the complete `CFS-0.1.0-Beta-win-x64` package to a writable local folder. Paths containing spaces are supported when commands remain quoted:

```powershell
& "C:\Users\Public\CFS 0.1.0 Beta\Cfs.App.exe"
```
...
The UI button **Register .cfs Double-Click** registers the current application for the current user. The packaged script supports quoted paths containing spaces:

```powershell
powershell -ExecutionPolicy Bypass -File ".\Register-CfsFileAssociation.ps1" -AppPath ".\Cfs.App.exe"
```
...
4. Delete the extracted `CFS-0.1.0-Beta-win-x64` folder or ZIP.
