# CFS 0.1.0 Beta data-safety guidance

## Backups and non-critical files

Keep a separate, independently accessible backup of every file placed in a `.cfs`. Use beta archives only for non-critical test files. CFS 0.1.0 Beta is not production-ready and must not be the sole storage location.
...
5. Retry **Save Mounted Changes** once the likely cause is removed.
